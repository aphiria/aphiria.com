...
EOF
    ...
    private function createTableIndices(): void
    {
        ...
        $statement = $this->pdo->prepare(
            <<<EOF
CREATE INDEX {$tableName}_english_lexeme_idx ON {$tableName} USING gin(english_lexemes)
EOF
        );
        $statement->execute();
        $statement = $this->pdo->prepare(
...
EOF
    ...
    /**
     * Drops the table that hold our docs
     */
...
EOF
    ...
    private function updateLexemes(): void
    {
        ...
        $statement = $this->pdo->prepare(
            <<<EOF
UPDATE {$tableName} SET english_lexemes = setweight(to_tsvector('english', COALESCE(inner_text, '')), html_element_weight::"char"), simple_lexemes = setweight(to_tsvector(COALESCE(inner_text, '')), html_element_weight::"char")
EOF
        );
        $statement->execute();
    }
