...
describe('Integration Tests', () => {
    ...
    beforeAll(() => {
        ...
        writeFileSync(join(testSourceDir, 'installation.md'), `
...
`);
        ...
        // Sample routing.md
        writeFileSync(join(testSourceDir, 'routing.md'), `
...
`);
    ...
    it('T039: compiles full docs directory and verifies output structure', async () => {
        ...
        const lexemesContent = readFileSync(join(testOutputDir, 'search', 'lexemes.ndjson'), 'utf8');
    ...
    it('T040: validates lexeme records match expected schema', async () => {
        ...
        lexemes.forEach(lexeme => {
            ...
            expect(['h1', 'h2', 'h3', 'h4', 'h5', 'p', 'li', 'blockquote']).toContain(lexeme.html_element_type);
        });
    });
    ...
    it('throws error when markdown files lack h1#doc-title', async () => {
        ...
        writeFileSync(join(invalidDir, 'invalid.md'), '<h1 id="wrong-id">Wrong ID</h1>\n\nContent without proper title.');

        await expect(buildDocs({
