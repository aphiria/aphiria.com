...
export function createNamespace(args: NamespaceArgs): NamespaceResult {
    ...
    // Create Namespace
    const namespace = new k8s.core.v1.Namespace(args.name, {
        metadata: {
            name: args.name,
            labels,
        },
    ...
    if (args.resourceQuota) {
        resourceQuota = new k8s.core.v1.ResourceQuota(`${args.name}-quota`, {
            metadata: {
    ...
    if (args.networkPolicy) {
        ...
        networkPolicy = new k8s.networking.v1.NetworkPolicy(`${args.name}-network-policy`, {
            metadata: {
    ...
    if (args.imagePullSecret) {
        imagePullSecret = new k8s.core.v1.Secret("ghcr-pull-secret", {
            metadata: {
