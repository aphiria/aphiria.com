...
describe("local stack", () => {
    ...
    it("should call createStack with local environment and minikube provider", async () => {
        mockCreateStack.mockReturnValue({
            config: {},
        });

        // Import the stack file to trigger execution
        await import("../../src/stacks/local");

        // Verify createStack was called with correct environment
        expect(mockCreateStack).toHaveBeenCalledWith("local", expect.any(Object));
        expect(mockCreateStack).toHaveBeenCalledTimes(1);

        // Verify the provider argument has correct context
        const providerArg = mockCreateStack.mock.calls[0][1];
