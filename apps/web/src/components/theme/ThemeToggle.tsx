"use client";

import { useTheme } from "@/lib/theme/useTheme";

/**
 * ThemeToggle component
 *
 * Icon button for toggling between light and dark themes.
 * - Shows sun icon in dark mode (click to switch to light)
 * - Shows moon icon in light mode (click to switch to dark)
 * - Centered in footer above "Aphiria was created by David Young"
 * - Fully accessible with keyboard navigation and ARIA labels
 */
export function ThemeToggle() {
    const { theme, toggleTheme } = useTheme();

    // Icon shows the mode the user will switch TO (action-oriented)
    const isSunIcon = theme === "dark"; // Sun in dark mode = switch to light
    const ariaLabel = theme === "dark" ? "Switch to light mode" : "Switch to dark mode";

    return (
        <div style={{ textAlign: "center", marginBottom: "1rem" }}>
            <button
                type="button"
                onClick={toggleTheme}
                aria-label={ariaLabel}
                style={{
                    background: "none",
                    border: "none",
                    cursor: "pointer",
                    fontSize: "1.5rem",
                    padding: "0.5rem",
                    color: "var(--color-text-primary)",
                    transition: "none", // No transitions to prevent flicker
                }}
            >
                {isSunIcon ? (
                    // Sun icon (light mode)
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        aria-hidden="true"
                    >
                        <circle cx="12" cy="12" r="5" />
                        <line x1="12" y1="1" x2="12" y2="3" />
                        <line x1="12" y1="21" x2="12" y2="23" />
                        <line x1="4.22" y1="4.22" x2="5.64" y2="5.64" />
                        <line x1="18.36" y1="18.36" x2="19.78" y2="19.78" />
                        <line x1="1" y1="12" x2="3" y2="12" />
                        <line x1="21" y1="12" x2="23" y2="12" />
                        <line x1="4.22" y1="19.78" x2="5.64" y2="18.36" />
                        <line x1="18.36" y1="5.64" x2="19.78" y2="4.22" />
                    </svg>
                ) : (
                    // Moon icon (dark mode)
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        aria-hidden="true"
                    >
                        <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
                    </svg>
                )}
            </button>
        </div>
    );
}
